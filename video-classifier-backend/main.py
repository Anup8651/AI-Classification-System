...
import os
import gc
os.environ['TF_ENABLE_ONEDNN_OPTS'] = '0'  # Hide oneDNN logs
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["https://ai-classification-system.vercel.app", "http://localhost:3000"],
    allow_credentials=True,
    allow_methods=["*"],
    allow_headers=["*"],
)
...
def create_imagenet_labels_file():
    """Create ImageNet labels file if needed (optional, since decode_predictions is built-in)"""
    if not os.path.exists("imagenet_labels.txt"):
        try:
            labels_path = keras.utils.get_file(
                'ImageNetLabels.txt',
...
def preprocess_frame(frame: np.ndarray) -> np.ndarray:
    """Preprocess a single frame (used only for image endpoint)"""
    frame_resized = cv2.resize(frame, (224, 224))
    frame_rgb = cv2.cvtColor(frame_resized, cv2.COLOR_BGR2RGB)
    frame_normalized = frame_rgb.astype(np.float32) / 255.0
    frame_batch = np.expand_dims(frame_normalized, axis=0)
    del frame_resized, frame_rgb
    return frame_batch

def classify_frame(frame: np.ndarray) -> List[Dict]:
    """Classify a single frame (used for /predict-frame endpoint)"""
    global model
    if model is None:
        raise HTTPException(status_code=503, detail="Model not loaded")
...
def extract_frames(video_path: str, num_frames: int = 10):
    """Extract frames sequentially – most reliable for Instagram/social media MP4s"""
    cap = cv2.VideoCapture(video_path)
    ...
    # Fallback for unreliable frame count
    if reported_total <= 0 or reported_total > 1_000_000:
        reported_total = 100_000
    
    duration = reported_total / fps if fps > 0 else 0
    
    logger.info(f"Video info: ~{reported_total} frames, {fps:.2f} FPS, {duration:.2f}s duration")
    
    frames = []
    frame_count = 0
    interval = max(1, reported_total // num_frames)
    ...
    actual_total_frames = frame_count
    
    if not frames:
        raise HTTPException(status_code=400, detail="Could not extract any frames")
    
    logger.info(f"Extracted {len(frames)} frames sequentially (read {actual_total_frames} total frames)")
    
    return frames, actual_total_frames, fps, duration
...
async def predict_video(
    file: UploadFile = File(...),
    num_frames: int = 5,  # Low default for speed on Vercel
    aggregation_method: str = "average"
):
    allowed_extensions = {'.mp4', '.avi', '.mov', '.mkv', '.flv', '.wmv', '.webm'}
    file_ext = Path(file.filename).suffix.lower()
    ...
    try:
        ...
        # Batch preprocess: BGR → RGB → float32 / 255
        rgb_frames = [cv2.cvtColor(frame, cv2.COLOR_BGR2RGB) for frame in frames]
        batch_array = np.array(rgb_frames, dtype=np.float32) / 255.0
        
        del frames, rgb_frames
        gc.collect()
        
        # Single batched inference
        logger.info("Running batched MobileNetV2 inference...")
        batch_predictions = model.predict(batch_array, verbose=0)
        
        # Decode all at once
        decoded_all = keras.applications.mobilenet_v2.decode_predictions(batch_predictions, top=5)
        
        # Build per-frame results
        frame_predictions = []
        frame_details = []
        
        for i, decoded_frame in enumerate(decoded_all):
            frame_result = []
            for rank, (imagenet_id, label, score) in enumerate(decoded_frame, 1):
                frame_result.append({
                    "rank": rank,
                    "class_id": imagenet_id,
                    "class_name": label,
            ...
            frame_details.append({
                "frame_index": i + 1,
                "timestamp": round((i + 1) / len(batch_array) * duration, 2) if duration > 0 else 0,
                "top_prediction": frame_result[0] if frame_result else None
            })
        ...
        response = {
            ...
            "video_info": {
                ...
                "fps": round(fps, 2),
                "duration_seconds": round(duration, 2),
                "frames_analyzed": len(batch_array)
            },
            "classification": {
                "aggregation_method": aggregation_method,
                "primary_prediction": aggregated_result["top_prediction"],
                "alternative_predictions": aggregated_result["all_predictions"][1:6],
                "confidence_breakdown": {
                    "very_high": len([p for p in aggregated_result["all_predictions"] if p["confidence_percent"] >= 80]),
                    "high": len([p for p in aggregated_result["all_predictions"] if 60 <= p["confidence_percent"] < 80]),
                    "medium": len([p for p in aggregated_result["all_predictions"] if 40 <= p["confidence_percent"] < 60]),
            ...
            "frame_analysis": {
                "summary": f"Analyzed {len(batch_array)} frames",
                "frame_predictions": frame_details[:5]
            }
        }
